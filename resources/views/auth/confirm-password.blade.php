...
@section('title', 'Confirm Password | The Technical Editor')
...
<p class="font-body italic text-base md:text-lg text-secondary">Confirm your password</p>
</div>
...
<span class="material-symbols-outlined text-primary text-5xl md:text-6xl mb-4" data-icon="password">password</span>
</div>
<p class="font-body text-sm text-on-surface-variant mb-6">This is a secure area of the application. Please confirm your password before continuing.</p>
<form class="space-y-5" method="POST" action="{{ route('password.confirm.submit') }}">
@csrf
<div>
<label class="block font-sans-tight text-sm font-bold text-on-surface-variant mb-2" for="password">Password</label>
<div class="relative">
<div class="absolute inset-y-0 left-0 pl-3 md:pl-4 flex items-center pointer-events-none">
<span class="material-symbols-outlined text-outline text-lg md:text-xl" data-icon="lock">lock</span>
</div>
<input class="block w-full pl-10 md:pl-11 pr-4 py-3 md:py-3.5 bg-surface-container-low border-none rounded-lg text-on-surface font-sans-tight focus:ring-2 focus:ring-primary/20 focus:bg-surface-container-lowest transition-all duration-200 placeholder:text-outline/50 text-base md:text-base" id="password" name="password" placeholder="Enter your password" required="" type="password"/>
</div>
@error('password')
